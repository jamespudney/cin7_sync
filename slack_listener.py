...
def _build_slack_system_prompt(channel_intent: str) -> str:
    ...
    if channel_intent == "po_review":
        base += (
            ...
            "Memo / Note / Terms.\n"
            "2. **v2.67.196 — fallback for fresh POs:** if "
            "get_purchase_order returns matched=0 with a note "
            "about the local sync window, IMMEDIATELY call "
            "get_purchase_live with the same po_number. This "
            "hits CIN7's API directly and returns the lines "
            "even when the PO is minutes old and hasn't synced "
            "yet. Don't tell the user to paste lines — try the "
            "live fetch first.\n"
            "3. THEN run the same per-SKU analysis as case (a) "
            "for each line item returned.\n"
            "4. Only if BOTH get_purchase_order AND "
