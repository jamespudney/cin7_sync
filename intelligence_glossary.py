...
MTD/current-month demand for these SKUs depends on
...
source. Count outbound `Sale` + `Finished Goods` / `Assembly` rows as
demand. Report inbound `Advanced Purchase` / `Purchase` rows separately
and never net purchases against demand.
...
5. **Per-row override**: the buyer can flip any row's mode via the
   ...
   (manual)"). This changes the row's supplier/freight default shown as
   Vendor LT; Sku LT still wins if it is filled. The reorder qty
   recalculates with the new lead time on next refresh.
