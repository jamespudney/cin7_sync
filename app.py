...
with st.sidebar:
    ...
    st.caption(
        ...
        "body to 4KB per product, and logs find_products entry/"
        "exit so a future OOM is diagnosable from Render logs. "
        "Functionality unchanged from v2.67: new find_products "
        "tool unions CIN7 inventory with the Shopify product KB "
        "so 'what warm white strips do we have' surfaces Shopify-"
        "only families (White Lily, pure-white White Iris, "
        "Cardinal Flower, etc.) instead of silently omitting "
        "them. Each result is tagged source ∈ "
        "{cin7, shopify, both}; Shopify-only rows come back with "
        "stock_status='unknown' and a 'Found in Shopify; stock "
        "data not available' note. AI Assistant page shows a "
        "freshness banner if /data/shopify/ is missing or older "
        "than 48h. Family detector (Elite Gold, White Iris, "
        "White Lily, Decor, Cardinal Flower, Liatris, Baltic Ivy, "
        "Honey Suckle, Sierra, Smokies, Oslo, Slim8, Slim, "
        "PLW80, PLW70, Disa) is a placeholder until "
        "product_attributes (Tier-A1) ships. "
