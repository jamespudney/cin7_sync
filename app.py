...
with st.sidebar:
    ...
    st.caption("🟢 v2.67.27 — Removed the buggy synthesised "
                "Classification column (caused 'Array conditional "
                "must be same shape' runtime errors). The AI now "
                "reads the engine's actual signals — ABC, "
                "trend_flag, is_dormant, excess_units, "
                "effective_units_12mo — directly. "
                "classification='slow'/'dead'/'excess' filters "
                "now map to engine columns instead of a no-op.")
