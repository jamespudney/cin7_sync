    ...
    with main_col:
        ...
        if _user_question:
            ...
            _system_prompt = (
                ...
                # v2.67.23 — staff need stock qty FIRST, then the
                # rating. Lead each row with the on-hand number so
                # sales can see what's available; trend / class flags
                # follow as decision support.
                "**Stock-reduction signal (v2.67.23) — qty first, "
                "rating second, in every product list:** every "
                "find_products / search_products row carries:\n"
                "  - `stock` (the on-hand quantity — the FIRST "
                "thing sales staff need to see)\n"
                "  - `classification` ∈ "
                "{'active','slow','dead','excess'} — derived from "
                "is_dormant + excess_units + 12mo demand (RULES.md "
                "§3 + §4)\n"
                "  - `trend_flag` ∈ {'Stable','📈 Trend','🎯 "
                "Project','🔀 Mixed','📉 Decline'} — the Ordering "
                "page's sales-staff rating; concentrated demand → "
                "Project, falling momentum → Decline, broad "
                "acceleration → Trend\n"
                "Format every product row as `<SKU> — <Name> — "
                "**<stock> on hand** — <trend_flag if non-Stable> "
                "— <classification if non-active>`. Examples:\n"
                "  - `LEDIRIS2700-120-100M — White Iris 2700K "
                "120LEDs/m 100m roll — **1.02 on hand** — 📉 "
                "Decline — slow-mover`\n"
                "  - `LED-CFWW-3000K-24-3 — Cardinal Flower 3000K "
                "3m — **5.0 on hand** — Stable`\n"
                "Sales staff scan stock qty to know what's "
                "available and the rating to know what to push. "
                "Slow / dead / decline items are PRIORITY to sell "
