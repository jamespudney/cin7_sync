...
# ---------------------------------------------------------------------------
    ...
    # ----- AI Warning column (score-based, no LLM call per row) --------
    # Each SKU gets a 0-100 demand score computed from its
    # demand_signals (volume × quality × source × recency × conversion
    # − quality_penalty). The score → warning level mapping is in
    # demand_scoring.score_warning_level(). See docs/demand-scoring.md
    # for the formula.
    #
    # Performance: we batch-compute scores for ALL SKUs with at least
    # one signal in one DB scan (db.compute_demand_scores_batch). SKUs
    # without signals get no warning. For 11k products this is fast
    # because most rows are silent.
    try:
        _scores_by_sku = db.compute_demand_scores_batch(
            window_days=30, conversion_window_days=90)
    except Exception:
        _scores_by_sku = {}

    import demand_scoring as _ds_mod

    def _warning_for_row(row: dict) -> tuple:
        """Returns (level, short_text). level in {'high', 'medium',
        'watch', None}. Pure score-based now — no manual rule
        priority tree."""
        sku = str(row.get("SKU") or "")
        if not sku:
            return (None, "")
        score_dict = _scores_by_sku.get(sku)
        if not score_dict:
            return (None, "")  # no signals → no warning
        return _ds_mod.score_warning_level(
            score_dict,
            classification=str(row.get("Classification") or ""),
            reorder_suggested=float(row.get("ReorderSuggested") or 0),
        )
    ...
    with main_col:
        ...
        if _user_question:
            ...
            _system_prompt = (
                ...
                "'what are customers asking about?', 'what's getting "
                "attention this week?', 'what's the demand score for "
                "X?', 'is this real or a one-off?'. Signals are "
                "LEADING indicators — they show interest BEFORE it "
                "appears in sales. The demand score (0-100) combines "
                "volume + signal type + source credibility + recency "
                "+ conversion rate. Cite the score AND the confidence "
                "(0-1) — confidence < 0.5 means the score is "
                "preliminary. See docs/demand-scoring.md for "
                "the formula.\n\n"
                ...
                "**Stock-listing coverage rule (v2.67.28) — "
                "MANDATORY:** for any 'what do we have' / 'show "
                "me our stock' question, you MUST:\n"
                "  1. Pass `limit=200` (not the default 25). "
                "Stock answers are comprehensive by design — the "
                "buyer crew expects to see every parent / "
                "standalone product, not the first 25.\n"
                "  2. Pass `parents_only=true` AND "
                "`in_stock_only=true`.\n"
                "  3. Pass `exclude_types` for strip queries — "
                "['dimmer', 'controller', 'power supply', "
                "'channel', 'profile', 'diffuser', 'connector', "
                "'extension cable', 'amplifier', 'driver', "
                "'fixture', 'kit', 'module', 'accessory']. "
                "(search_products_by_text auto-unions this list "
                "for queries containing 'strip' as of v2.67.28, "
                "but pass it anyway as a safety net.)\n"
                "  4. After receiving results, VERIFY family "
                "coverage. Known warm-white-capable families: "
                "White Iris, White Lily, Cardinal Flower, "
                "Honey Suckle, Decor, Elite Gold, Liatris, "
                "Sierra, Smokies, Baltic Ivy, Sauna Pro, Glow67 "
                "Pro. If ANY of these are absent from your "
                "result and the user asked for warm-white, "
                "investigate (call again with broader filters, "
                "or check the family explicitly via "
                "family=FAMILYNAME). Do NOT silently omit a "
                "known family — say 'White Iris not in result, "
                "investigating' and call the tool again.\n"
                "  5. Group the answer by family. Within each "
                "family, list every parent SKU with its OnHand "
                "qty + ABC + trend_flag (when non-Stable) + "
                "is_dormant flag (when true).\n"
                "  - **SHIPMENT-TIMING question (v2.67.46) — "
                "OVERRIDES STOCK intent**: when the user asks "
                "'when is X coming / arriving / due / next "
                "shipment', 'when do we get more X', 'what's "
                "the ETA on X', 'is there anything incoming for "
                "X', 'what's on order for X' — even if the "
                "STOCK intent chip is set, these questions need "
                "incoming-PO data, NOT current-stock listings. "
                "→ CALL `get_incoming_stock` with sku=<resolved "
                "specific SKU> OR family=<family-prefix>. If you "
                "don't have an exact SKU, first call "
                "`search_products_by_text` to resolve the user's "
                "phrase to one or more SKUs, THEN call "
                "`get_incoming_stock` for each (or with the "
                "family prefix). Surface the buyer's `comments` "
                ...
                "rule as POs. **Proactive ShipStation join "
                "(v2.67.55)**: ShipStation's shipment_number IS "
                "the CIN7 invoice_number (INV-XXXXX) — when "
                "get_sale_order returns a sale with an "
                "invoice_number, call `get_shipping_details` with "
                "order_number=<that invoice_number> to give the "
                "joined view (shipping cost, carrier, tracking, "
                "ship-to address). Don't say 'check ShipStation "
                "yourself'; do the join. Same proactive pattern "
                "as the Shopify-side join.\n"
                ...
                "**Natural-join key (v2.67.55)**: ShipStation's "
                "shipment_number = CIN7 invoice_number (INV-"
                "XXXXX). So a user asking 'where's INV-53141' "
                "or 'has invoice 53141 shipped' is best answered "
                "by passing `order_number=INV-53141` (with or "
                "without the INV- prefix). The same key joins "
                "to CIN7 sales — if the user asks the broader "
                "question ('what did Acme buy AND has it "
                "shipped'), call get_sale_order first, then "
                "get_shipping_details on the resulting "
                "invoice_number for the full picture.\n"
                ...
                # that failure mode.
                "**Always produce a final text answer (v2.67.46) — "
                "MANDATORY backstop:** no matter how many tools "
                "you call, you MUST produce at least one text "
                "response in the conversation BEFORE you stop "
                "calling tools. If a tool returns matched=0 or "
                "an error, that's still answerable — say what "
                "you searched for, what came back, and (when "
                "relevant) suggest a follow-up. Never finish a "
                "turn with only tool calls and no text — the "
                "Streamlit app shows the user 'I couldn't answer "
                "that' if there's no text accumulated. Examples:\n"
                "  - get_incoming_stock returns matched=0: 'No "
                "open POs for LED-89030020-2 right now. Last "
                "received stock came in on YYYY-MM-DD per the "
                "Stock Explorer page.'\n"
                "  - search_products_by_text returns matched=0: "
                "'No SKUs match \"slim8 silver\" with the "
                "current filters. Want me to broaden to all "
                "Slim8 products?'\n"
                ...
                "promote, or discontinue — every parent SKU is "
                "a distinct product they need to see. Group by "
                "family for readability, but every SKU must "
                "appear by name with its on-hand quantity. If "
                "`matched` is very large (>40), still list every "
                "SKU — long is the right shape here, not a "
                "summary. Cite the `matched` count up front so "
                "the user can sanity-check coverage against what "
                "they expect.\n\n"
                ...
                "**Parent-orderable SKUs only (v2.67.22 — "
                "default behaviour of find_products):** by default, "
                "find_products returns ONLY the supplier-orderable "
                "parent SKUs — child variants (per-foot cuts, "
                "voltage variants made via BOM, fractional-source "
                "SKUs) are hidden. Example: for the White Iris "
                "2700K family, you'll see LEDIRIS2700-120-100M "
                "(the 100m bulk roll we order from Neonica) but "
                "NOT LEDIRIS2700-120-0305 (the per-foot retail "
                "cut, made by assembling fractions of the parent). "
                "This matches the Ordering page's PO-suggestion "
                "logic. If the user explicitly asks 'show me "
                "every variant / every length / every voltage', "
                "pass parents_only=false to widen the answer; "
                "otherwise leave it default. Never silently omit "
                "the parents_only behaviour from your answer — "
                "if the user seems to expect a specific child "
                "SKU and it's missing, mention that "
                "parents_only=true hides it and offer to re-run "
                "with parents_only=false.\n\n"
                ...
                "carries the raw engine columns. Read them "
                "directly to flag stock-reduction candidates and "
                "rate stock for sales staff:\n"
                "  - `OnHand` — on-hand quantity. Lead every row "
                "with this number; it's what sales staff scan.\n"
                ...
                "slow-mover flag. True when 90d activity dropped "
                "vs the 12mo baseline. Render as 'slow-moving' in "
                "answers.\n"
                ...
                "**<OnHand> on hand** — ABC=<A/B/C>` with "
                "trend_flag (when non-Stable) and a slow/dead/"
                "excess flag appended when relevant. Examples:\n"
                "  - `LEDIRIS2700-120-100M — White Iris 2700K — "
                "**1.02 on hand** — ABC=B — 📉 Decline — slow-"
                "moving (is_dormant)`\n"
                "  - `LED-CFWW-3000K-24-3 — Cardinal Flower 3000K "
                "— **5.0 on hand** — ABC=A`\n"
                "Filtering: pass classification='slow' to filter "
                "to is_dormant=true rows; classification='dead' "
                "for OnHand>0 with zero 12mo demand; "
                "classification='excess' for excess_units>0. "
                "These are now wired to the engine columns "
                "directly (v2.67.27 — earlier versions tried to "
                "synthesise a Classification column on the fly "
                "which caused runtime errors and duplicated work "
                "the engine already does).\n"
                "Sales staff scan OnHand for availability and "
                "trend_flag/is_dormant for what to PUSH. Slow / "
