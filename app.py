...
with st.sidebar:
    ...
    st.caption("🟢 v2.67.22 — find_products now returns "
                "parent-orderable SKUs only (matches Ordering "
                "page logic) and tags each row with "
                "active/slow/dead/excess so the crew can spot "
                "stock-reduction candidates inline.")
    ...
    with main_col:
        ...
        if _user_question:
            ...
            _system_prompt = (
                ...
                # v2.67.22 — slow-mover surfacing. The big strategic
                # driver behind this app is shrinking stock holding,
                # so the AI must call out slow / dead / excess SKUs
                # whenever they appear in product-discovery results.
                "**Stock-reduction signal (v2.67.22) — surface "
                "slow movers in every product list:** every "
                "find_products / search_products row now carries a "
                "`classification` field — one of 'active', 'slow', "
                "'dead', or 'excess'. The mapping mirrors RULES.md "
                "§3 + §4: 'dead' = holding stock with zero 12-month "
                "effective demand; 'slow' = dormant (90d activity "
                "dropped vs 12mo baseline) with stock; 'excess' = "
                "over-target stock; 'active' = otherwise. **When "
                "answering product-discovery questions, prefix or "
                "annotate each non-'active' SKU with its flag** so "
                "the buyer crew can see at a glance which items "
                "are ripe for promotion / discount / discontinue. "
                "Format: `LEDIRIS2700-120-100M (1.02 pcs) ⚠️ "
                "slow-mover` or `LED-DECOR-2.6W-24V-2700K-IP20 "
                "(2.0 pcs) 🔴 dead stock`. The big strategic driver "
                "of this app is shrinking stock holding — every "
                "answer that touches inventory should help the "
                "buyer see which SKUs to reduce.\n\n"
